...
void Base::Init()
{
	...
	case MinecraftVersion::UNKNOWN:
		...
		MessageBoxA(nullptr, "Unknown Minecraft version detected", "MBTH", MB_OK | MB_ICONERROR);
		Logger::Shutdown();
		MH_Uninitialize();
		Java::Shutdown();
	...
	if (!HotSpot::Init())
	{
		...
		MessageBoxA(nullptr, "Failed to initialize HotSpot", "MBTH", MB_OK | MB_ICONERROR);
		Logger::Shutdown();
		MH_Uninitialize();
		Java::Shutdown();
		FreeLibraryAndExitThread(Main::hModule, 0);
	...
	while (Base::m_running)
	{
		...
		if (Java::env->PushLocalFrame(1000) >= 0)
		{
