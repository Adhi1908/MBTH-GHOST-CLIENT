...
struct settings
{
	...
	// KillAura — LiquidBounce-equivalent. See KILLAURA_DESIGN.md.
	// v0.1: visible rotation publish via SetAngles. v0.2: silent via JVMTI sendPacket hook.
	inline static bool  KA_Enabled            = false;
	inline static int   KA_Key                = 0x0;
	inline static float KA_Range              = 3.5f;
	inline static float KA_Fov                = 180.0f;
	inline static float KA_HSpeed             = 180.0f;   // yaw deg/tick cap
	inline static float KA_VSpeed             = 180.0f;   // pitch deg/tick cap
	inline static float KA_MinRotationDiff    = 2.0f;
	inline static bool  KA_Legitimize         = true;
	inline static bool  KA_SimulateShortStop  = false;
	inline static bool  KA_RaycastCheck       = true;
	inline static bool  KA_OnlyOnLMB          = false;
	inline static bool  KA_WeaponOnly         = true;
	inline static bool  KA_DisableWhenFlying  = true;
	inline static bool  KA_IgnoreFriends      = true;
	inline static bool  KA_IgnoreInvisible    = true;
	inline static int   KA_MinCps             = 6;
	inline static int   KA_MaxCps             = 11;
	inline static int   KA_TargetPriority     = 0;
	inline static const char* KA_TargetPriorityList[3]{ "Distance", "Health", "Closest to Crosshair" };
	inline static bool  KA_FovCircle          = false;
	inline static float KA_FovCircleColor[4]{ 1.f, 0.f, 0.f, 1.f };

	// TriggerBot — fires when crosshair is already on a target (no rotation surface).
	inline static bool  TBot_Enabled         = false;
	inline static int   TBot_Key             = 0x0;
	inline static int   TBot_MinCps          = 6;
	inline static int   TBot_MaxCps          = 11;
	inline static int   TBot_HoldDelay       = 150;   // ms — reaction-time floor
	inline static bool  TBot_OnlyOnLMB       = true;
	inline static bool  TBot_WeaponOnly      = true;
	inline static bool  TBot_PlayersOnly     = true;
	inline static bool  TBot_IgnoreFriends   = true;
	inline static bool  TBot_IgnoreInvisible = true;
	...
	inline static float Reach_ReachDistance   = 3.5f;   // 3.0 = vanilla, 6.0 = ridiculous
	inline static bool  Reach_OnlyOnLMB       = true;   // Only attack while LMB is held
	inline static bool  Reach_WeaponOnly      = true;   // Only act with sword/axe/etc.
	inline static bool  Reach_VisibilityCheck = true;   // Skip targets behind walls
	inline static bool  Reach_IgnoreFriends   = true;
	inline static int   Reach_MinCps          = 6;
	inline static int   Reach_MaxCps          = 12;
	...
	// Auto Tool
	inline static bool AT_Enabled = false;
	inline static int  AT_Key = 0x0;
	inline static bool AT_HotkeyBack = true;   // Switch back to original slot when finished
	inline static int  AT_MaxDelay = 0;        // ms delay before switching (0 = instant)
	inline static bool AT_OnlyWhenHoldingTool = false; // Only act if currently holding a tool/sword

	// Notification sounds
	inline static bool Notif_SoundsEnabled = true;
	inline static bool Notif_SoundOnInfo    = false; // info popups are frequent; default off
	inline static bool Notif_SoundOnWarning = true;
	inline static bool Notif_SoundOnError   = true;
